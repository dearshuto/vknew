...
impl Renderer {
        ...
        W: HasWindowHandle + HasDisplayHandle,
    {
        ...
        let (vs_shader_module, fs_shader_module) = {
            ...
            void main()
            {
                vec2 position = u_Scale * v_Position;
        ...
        ";
            ...
            let vs_create_info = ash::vk::ShaderModuleCreateInfo::default().code(&vs_code);
            let fs_create_info = ash::vk::ShaderModuleCreateInfo::default().code(&fs_code);
            (
                unsafe { device.create_shader_module(&vs_create_info, None) }.unwrap(),
                unsafe { device.create_shader_module(&fs_create_info, None) }.unwrap(),
            )
        };
